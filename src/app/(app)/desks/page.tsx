...
export default async function DesksPage() {
  ...
  const { data: assignedProfiles } = assignedUserIds.length
    ? await admin.from("profiles").select("id, full_name, area").in("id", assignedUserIds)
    : { data: [] };
  ...
  const enrichedDesks: DeskWithStatus[] = (desks ?? []).map((desk) => ({
    ...
    assigned_profile: desk.assigned_user_id ? profileById[desk.assigned_user_id] ?? null : null,
    owner_day_status: desk.assigned_user_id ? (statusByUser[desk.assigned_user_id] ?? null) : null,
  }));
